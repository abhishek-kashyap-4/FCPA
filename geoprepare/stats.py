...
from .util.raster import raster as raster
from .util.raster import array as rasterarray
...
def geom_extract(
    ...
    if indicator_ds.driver == "ENVI":
        #A_K_?? 
        import envipyengine as envi #added this.
    ...
    if _dtype_conversion:
        #A_K_ checked to rasterarray from raster. see imports.
        indicator_arr = rasterarray.arr_unpack(indicator_arr, **_dtype_conversion)
    ...
    if afi:
        ...
        if _dtype_conversion:
            #A_K_ changed from raster to rasterarray. see imports.
            afi_arr = rasterarray.arr_unpack(afi_arr, **_dtype_conversion)
    ...
    if any(val in ("min", "max", "mean", "sum" "std") for val in stats_out):
        ...
        #A_K_ changed to rasterarray from raster. Check imports. 
        output["stats"] = rasterarray.arr_stats(
